<?php
/*
Widget Name: CE &#9829; s2 Thumbnail
Description: Display a thumbnail of a comic, either newest or first or random, excluding ones that are still on drip
Author: Philip M. Hofer (Frumph), with modifications by Chrisitna "Smudge" Hanson
Author URI: http://frumph.net/
Version: 1.3, v.1 fork
*/

## Note: Think about putting in a if(current_user_has_access()) to control wither the lockout is applied or not

defined('ABSPATH') or die("No script kiddies please!");
	
class ce2s_thumbnail_widget extends WP_Widget {

	function ce2s_thumbnail_widget() {
		...
		$this->WP_Widget(__CLASS__, __('CE &#9829; s2 - Thumbnail', 'comiceasel'), $widget_ops);
	}
	...
	function widget($args, $instance) {
		...
		if ($instance['inhistory'])
			...
			{
			$drip_time = '1 month ago';
			$ce_s2_date_query = array('column' => 'post_date_gmt','before' => $drip_time,);
			}
			
		if ($instance['thumbchapt'] !== 'All') { $chaptinfo = $instance['thumbchapt']; } else { $chapinfo = NULL; }
		if ($instance['first']) { $order = 'ASC'; } else { $order = 'DESC'; }
		if ($instance['random']) { $random = 'rand'; } else { $random = NULL; }
...
function ce2s_thumbnail_widget_register() {
	register_widget('ce2s_thumbnail_widget');
}

add_action('widgets_init', 'ce2s_thumbnail_widget_register');
?>
